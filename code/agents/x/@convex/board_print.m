...
function board_print(obj,texts,boards)
    ...
    for i = 1:n
        ...
        action{i} = num2str(reshape(board(end+1-obj.options.n_action:end),obj.options.size(2),obj.options.n_action./obj.options.size(2))');
    ...
    % print strings
    cprintf([0,1,0],[texts,'\n']);
    for i = 1:size(status,1), fprintf(         [status(i,:),'\n']); end
    for i = 1:size(action,1), cprintf('*black',[action(i,:),'\n']); end
    fprintf('\n');
